...
### V4 trace failure and V5 correction

The 100k local V4 curriculum reached 86.96% generated-answer accuracy at phase
epoch 60. Both held-out addition operations reached 81.82%, but only 4/22
addition traces were exact (18.18%, versus the 70% gate). The other phase-one
trace families reached 91.67% and 75%. Inspection showed that most failed
addition rows had the correct final answer but an off-by-one expanded count-on
list. This was not evidence for insufficient tower capacity or too few epochs.

The V4 trace builder marked only one final work result as `compute`; generated
intermediate sequence values were classified as numeric copies by the collator.
It also required the byte model to emit a variable-length list such as
`[4,5,6]`, which is unnecessary for verifying addition and does not scale to
longer procedures.

The versioned V5 recipe therefore requires:

- compact executable count-on steps (`start`, `add`, cumulative `end`) rather
  than enumerating every intermediate integer;
- trace text and role spans generated by one annotated canonical serializer,
  with every derived intermediate value labelled `compute`;
- a hybrid objective combining ordinary full-target cross-entropy with the
  role-balanced computation objective, so syntax fidelity is not traded away;
- parsed, executable semantic trace acceptance as the promotion gate, while raw
  canonical-string equality remains a stricter diagnostic;
- a fresh dataset identity and full scratch run. V4 checkpoints are not resumed
  into V5 because both the target representation and objective changed.

For later algebra, calculus and proof-oriented phases, traces must remain typed,
bounded and executable. Never encode an algorithm by expanding an unbounded
number of low-level output tokens when a compact verified state transition is
available.
