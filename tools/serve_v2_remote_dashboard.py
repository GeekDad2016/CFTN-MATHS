...
markers = ("run_v2.py", "prepare_v2_data", "train_math_tower", "recover_v2_math", "train_v2_dispatcher", "train_v1_3_integration", "evaluate_v2", "evaluate_v1_3", "train_v2_verified_school", "train_v2_full_supervision", "run_v2_math_curriculum")
sensitive_names = {
    "api_key",
    "authorization",
    "password",
...
recovery_roots = sorted(
    {
        path
        for pattern in (
            "math_competency_curriculum*",
            "math_scratch_curriculum*",
...
for candidate_root in recovery_roots:
    ...
    if candidate_contract and (candidate_root.name.startswith(("math_full_supervision", "math_competency_curriculum", "math_scratch_curriculum")) or str(candidate_status.get("state") or "starting") not in recovery_terminal_states):
...
if recovery_root is not None:
    ...
    training_contract = redact({
        ...
        "initialization": recovery_contract.get("initialization"),
        "full_supervision": recovery_contract.get("format") in {"cftn_full_math_training_v1", "cftn_full_math_training_v2", "cftn_full_math_training_v3", "cftn_full_math_training_v4"},
        "competency_curriculum": recovery_contract.get("format") in {"cftn_full_math_training_v2", "cftn_full_math_training_v3", "cftn_full_math_training_v4"},
        "original_source_sha256": recovery_contract.get("original_source_sha256"),
        "remaining_pipeline_enabled": recovery_contract.get("remaining_pipeline_enabled"),
        "retention_baseline": recovery_contract.get("retention_baseline"),
        "zero_update_entrance": recovery_contract.get("zero_update_entrance"),
        "preservation_distillation": recovery_contract.get("preservation_distillation"),
    ...
    if recovery_contract.get("data_root"):
        data_root = Path(recovery_contract["data_root"])
school_trial = {}
for trial_root in sorted(artifact_root.glob("math_verified_school_trial*"), key=modified_unix, reverse=True):
    if recovery_root is not None and recovery_root.name.startswith(("math_full_supervision", "math_competency_curriculum", "math_scratch_curriculum")):
        break
...
for stage_name, directory in stage_directories.items():
    ...
    retention_baseline = redact(read_json(stage_root / "retention_baseline.json"))
    entrance_evaluations = redact(read_json(stage_root / "entrance_evaluations.json"))
    ...
    if status or metrics or summary or report or stage_name in pipeline.get("stages", {}):
        stage_artifacts[stage_name] = {
            ...
            "summary": summary,
            "report": report,
            "initialization": recovery_contract.get("initialization") if stage_name == recovery_stage else None,
            "retention_baseline": retention_baseline,
            "entrance_evaluations": entrance_evaluations,
        }
if school_trial:
...
if current_root is not None:
    ...
    scope_override = read_json(current_root / "generation_panel_scope_override.json")
    generation_scope = str(scope_override.get("scope") or "all_configured_v1")
    active_phase_name = None
    if generation_scope == "phase_required_v1" and configured_panels:
        phase_state = live_metrics.get("competency_curriculum_state") or (
            current_metrics[-1].get("competency_curriculum_state", {})
            if current_metrics else {}
        )
        phase_index = int(phase_state.get("phase_index") or 0)
        phases = current_contract.get("phases") or []
        if 0 <= phase_index < len(phases):
            phase = phases[phase_index]
            active_phase_name = str(phase.get("name") or "")
            required = {str(phase.get("primary_generation_panel", "validation"))}
            for key in ("minimum_generation_accuracy_by_panel", "minimum_valid_rate_by_panel"):
                thresholds = phase.get(key) or {}
                if isinstance(thresholds, dict):
                    required.update(str(name) for name in thresholds)
            configured_panels = [
                panel for panel in configured_panels
                if str(panel.get("name") or "") in required
            ]
    batches_complete = (
...
if school_trial or (recovery_root is not None and stage == recovery_stage):
    stdout_paths = sorted(artifact_root.glob(f"{stage}*.stdout.log"), key=lambda p: p.stat().st_mtime, reverse=True)
    stderr_paths = sorted(artifact_root.glob(f"{stage}*.stderr.log"), key=lambda p: p.stat().st_mtime, reverse=True)
    stdout = tail(stdout_paths[0]) if stdout_paths else {}
    stderr = tail(stderr_paths[0]) if stderr_paths else {}
else:
...
<div class="card wide"><h2>How to read validation</h2><div id="validationScope"></div></div>
<div class="card wide"><h2>Current-phase learning</h2><div id="phaseValidation"></div></div>
<div class="card wide"><h2>Retention versus source baseline</h2><div id="retentionValidation"></div></div>
<div class="card wide"><h2>Future curriculum probes</h2><div id="futureValidation"></div></div>
<div class="card wide"><h2>Global teacher-forced diagnostics</h2><div id="breakdowns"></div></div>
<div class="card wide"><h2>Generation examples</h2><div id="generation"></div></div>
...
<div class="card wide"><h2>GPU</h2><div id="gpu"></div></div>
<div class="card wide"><h2>Processes</h2><div id="processes"></div></div>
<div class="card wide"><h2>Checkpoints</h2><div id="checkpoints"></div></div>
<div class="card wide"><h2>W&amp;B</h2><div id="wandb"></div></div>
<div class="card wide"><h2>Current stdout</h2><pre id="stdout"></pre></div>
<div class="card wide"><h2>Current stderr</h2><pre id="stderr"></pre></div>
<script>
...
function tbl(rows,heads,raw=false){return rows?.length?'<div class="table-scroll"><table><tr>'+heads.map(x=>'<th>'+esc(x)+'</th>').join('')+'</tr>'+rows.map(r=>'<tr>'+r.map(x=>'<td'+(String(x).length>45?' class="wrap"':'')+'>'+(raw?x:esc(x))+'</td>').join('')+'</tr>').join('')+'</table></div>':'<span class="muted">None yet.</span>'}
...
function curriculumState(d,status,live,rows,epochProgress){let c=d.training_contract?.curriculum||{},phases=c.phases||[],last=latest(rows),epoch=Number(status.epoch??live.epoch??last.epoch??0),dynamic=String(c.transition_policy||'').startsWith('competency_gated_v'),saved=last.competency_curriculum_state||{},entrance=d.current_stage_artifact?.entrance_evaluations?.resulting_curriculum_state||{},activeIndex=dynamic?Number(saved.phase_index??entrance.phase_index??0):phases.findIndex(x=>epoch<=Number(x.through_epoch));if(activeIndex<0&&phases.length)activeIndex=phases.length-1;let active=phases[activeIndex]||{},start=activeIndex>0?Number(phases[activeIndex-1].through_epoch)+1:1,count=dynamic?Math.max(1,Number(active.maximum_epochs||1)):Math.max(1,Number(active.through_epoch)-start+1),completedInPhase=dynamic?Number(saved.phase_epoch||0):epoch-start,phaseProgress=Math.max(0,Math.min(1,(completedInPhase+Number(epochProgress||0))/count)),epochTimes=rows.map(r=>Number(get(r,'timing.epoch_seconds',NaN))),typical=median(epochTimes),remaining=Math.max(0,count-completedInPhase-Number(epochProgress||0));return {config:c,phases,activeIndex,active,start,epoch,dynamic,completedInPhase,phaseProgress,typical,transitionEta:Number.isFinite(typical)?remaining*typical:NaN}}
function entrancePanel(a,contract){if(contract?.initialization?.mode==='random_scratch_v1'||a?.initialization?.mode==='random_scratch_v1')return '<div class=callout><span class="pill ok">Random scratch initialization</span><p>No inherited checkpoint, teacher, or zero-update phase skip is used. Every phase must be learned and pass repeated direct generation gates.</p></div>';let report=a.entrance_evaluations||{},items=report.evaluations||[];if(!items.length)return '<p class=muted>Zero-update entrance checks have not completed yet.</p>';let rows=items.map(x=>[String(x.phase||'').replaceAll('_',' '),x.zero_optimizer_updates?'0':'—',x.acceptance?.pass?'PASS':'MISS',x.skipped?'Skipped as already mastered':'First unmet phase — training starts here']);return '<h3>Zero-update phase entrance</h3>'+tbl(rows,['Phase','Optimizer updates','All unchanged gates','Decision'])+'<p class=muted>Only accepted non-final phases may be skipped. The final phase always requires fresh repeated evidence.</p>'}
function competencyAssessment(d,p,a,issues,curr){let rows=a.metrics||[],last=latest(rows);if(!rows.length)return {level:'warn',label:'Too early to assess',detail:'No completed phase validation is available yet.',html:'<span class=muted>Assessment starts after the first completed curriculum validation.</span>'};let acceptance=last.curriculum_acceptance||{},checks=acceptance.checks||{},phaseChecks=Object.entries(checks).filter(([name])=>name!=='broad_retention'),phasePassed=phaseChecks.length>0&&phaseChecks.every(([,value])=>value.pass===true),retention=checks.broad_retention||{},hasRetention=Object.prototype.hasOwnProperty.call(checks,'broad_retention'),retentionPassed=!hasRetention||retention.pass===true,transition=last.curriculum_transition||{},phaseName=String(transition.phase||acceptance.phase||curr.active.name||'current phase').replaceAll('_',' '),streak=Number(transition.consecutive_passes??get(last,'competency_curriculum_state.consecutive_passes',0)),required=Number(transition.required_consecutive_passes??curr.active.advance_after_consecutive_passes??1),phaseEpoch=Number(transition.phase_epoch??acceptance.phase_epoch??get(last,'competency_curriculum_state.phase_epoch',0)),maximum=Number(transition.maximum_epochs??curr.active.maximum_epochs??0),level=issues.length?'bad':transition.failed?'bad':acceptance.pass?'ok':phasePassed&&hasRetention&&!retentionPassed?'warn':'warn',label=issues.length?'Infrastructure attention required':transition.failed?'Phase stopped: acceptance failed':transition.advance?'Phase accepted — advancing':acceptance.pass?(hasRetention?'All phase and retention gates pass':'All current-phase gates pass'):phasePassed&&hasRetention&&!retentionPassed?'Current phase learned; retention below floor':'Current-phase gates still learning',detail=phasePassed&&hasRetention&&!retentionPassed?'The skills taught in this phase pass their generation gates, but the broad replay panel has fallen below the protected source-checkpoint floor.':hasRetention?'The curriculum decision is based on generated answers for the active phase plus protected broad retention, not the global 12,000-example teacher-forced table.':'This random-initialized tower advances on direct generated-answer and executable-trace gates; cumulative replay in each later phase protects earlier skills.',next=transition.failed?'The run is fail-closed. Diagnose the unmet phase gates before continuing.':transition.advance?'The next curriculum phase begins from the accepted checkpoint.':`A phase transition needs ${required} consecutive complete passes. Current streak: ${streak}; phase epoch ${phaseEpoch}${maximum?' of at most '+maximum:''}.`;let rowsOut=[['Active phase',phaseName,'Acceptance scope'],['Current-phase checks',phaseChecks.filter(([,v])=>v.pass).length+' / '+phaseChecks.length,phasePassed?'PASS':'Still learning'],['Inherited retention',hasRetention&&numeric(retention.observed)?pct(retention.observed):'Not applicable',hasRetention&&numeric(retention.minimum)?'floor '+pct(retention.minimum):'Random scratch; cumulative replay'],['Consecutive complete passes',streak+' / '+required,transition.advance?'Transition approved':'No transition yet']];return {level,label,detail,html:`<div class="callout ${acceptance.pass?'okay':''}"><span class="pill ${level}">${esc(label)}</span><p>${esc(detail)}</p><p><b>Next decision:</b> ${esc(next)}</p></div>`+tbl(rowsOut,['Signal','Observed','Meaning'])}}
function trainingAssessment(d,p,a,issues,curr){if(curr.dynamic)return competencyAssessment(d,p,a,issues,curr);let rows=a.metrics||[];if(!rows.length)return {level:'warn',label:'Too early to assess',detail:'No completed validation epoch is available yet.',html:'<span class=muted>Assessment starts after epoch 1 validation.</span>'};if(rows.length===1){let only=rows[0],phaseName=String(curr.active.name||'current phase').replaceAll('_',' ');return {level:issues.length?'bad':'warn',label:issues.length?'Infrastructure attention required':'Awaiting second validation',detail:'One completed epoch establishes a baseline but cannot establish a trend.',html:`<div class=callout><span class="pill ${issues.length?'bad':'warn'}">${issues.length?'Infrastructure attention required':'Awaiting second validation'}</span><p>Epoch ${n(only.epoch)} establishes the first validation baseline. A direction can be reported after epoch 2 validation.</p><p>The model is in <b>${esc(phaseName)}</b>; the authoritative capacity screen remains epoch ${n(curr.active.through_epoch)}.</p></div>`}}let first=rows[0],last=latest(rows),train0=Number(first.train_loss),train1=Number(last.train_loss),val0=Number(get(first,'validation.loss',NaN)),val1=Number(get(last,'validation.loss',NaN)),tok0=Number(get(first,'validation.teacher_forced_token_accuracy',NaN)),tok1=Number(get(last,'validation.teacher_forced_token_accuracy',NaN)),seq0=Number(get(first,'validation.teacher_forced_sequence_accuracy',NaN)),seq1=Number(get(last,'validation.teacher_forced_sequence_accuracy',NaN)),trainChange=100*(train1/train0-1),valChange=100*(val1/val0-1),tokenPoints=100*(tok1-tok0),sequencePoints=100*(seq1-seq0),learning=train1<train0&&seq1>seq0,pressure=valChange>15||tokenPoints < -1,level=issues.length?'bad':(learning&&pressure?'warn':learning?'ok':'warn'),label=issues.length?'Infrastructure attention required':(learning&&pressure?'Acceptable to continue — watch validation':learning?'Healthy learning trend':'Learning signal is not yet convincing'),patience=Number(last.patience??0),patienceLimit=Number(d.training_contract?.math_training?.early_stop_patience??0),times=rows.map(r=>({epoch:r.epoch,seconds:Number(get(r,'timing.epoch_seconds',NaN))})),typical=median(times.map(x=>x.seconds)),outliers=times.filter(x=>Number.isFinite(typical)&&x.seconds>typical*1.75),phaseName=String(curr.active.name||'current phase').replaceAll('_',' '),detail=learning?'Training loss is falling and exact-sequence accuracy is materially above epoch 1.':'The completed epochs do not yet show both falling training loss and improving exact-sequence accuracy.',next=curr.activeIndex<=1&&curr.epoch<=15&&pressure?'The decisive check is 3–5 completed epochs into difficulty 2: by epochs 14–15, validation loss should stabilize or fall and token accuracy should hold or recover while sequence accuracy rises.':'Continue to judge the run on validation accuracy, validation loss, and accepted-checkpoint selection together.',valInterpretation=valChange<=0?'Improving':valChange>15?'Under pressure':'Slightly elevated',tokenInterpretation=tokenPoints>=0?'Improving on full mixed validation':'Full mixed validation under watch';let metricRows=[['Train loss',n(train0)+' → '+n(train1),signed(trainChange),'Lower is better'],['Validation loss',n(val0)+' → '+n(val1),signed(valChange),valInterpretation],['Token accuracy',pct(tok0)+' → '+pct(tok1),signed(tokenPoints,2,' pp'),tokenInterpretation],['Exact-sequence accuracy',pct(seq0)+' → '+pct(seq1),signed(sequencePoints,2,' pp'),'Checkpoint signal'],['Early-stop patience',patience+' / '+n(patienceLimit),'',patience<=1?'Normal single-epoch noise':'Watch consecutive non-improvements']];let html=`<div class=callout><span class="pill ${level}">${esc(label)}</span><p>${esc(detail)} The model is still in <b>${esc(phaseName)}</b>, while validation covers all difficulty levels.</p><p><b>Next decision point:</b> ${esc(next)}</p></div>`+tbl(metricRows,['Signal','Epoch 1 → latest','Change','Interpretation'])+`<p class=muted>Typical epoch: ${dur(typical)}. ${outliers.length?esc(outliers.map(x=>'Epoch '+x.epoch).join(', '))+' ran unusually slowly; subsequent timing recovered.':'No material timing outlier detected.'}</p>`;return {level,label,detail,html}}
function gateValue(name,value){return /(accuracy|rate)/i.test(name)?pct(value):n(value)}
function acceptancePanel(last,curr){if(!last?.epoch)return '<span class=muted>No completed validation epoch yet.</span>';let acceptance=last.curriculum_acceptance||{},checks=acceptance.checks||{},entries=Object.entries(checks),passed=entries.filter(([,v])=>v.pass).length,rows=entries.map(([name,v])=>[name.replaceAll(':',' · ').replaceAll('_',' '),gateValue(name,v.observed),numeric(v.minimum)?'≥ '+gateValue(name,v.minimum):numeric(v.maximum)?'≤ '+gateValue(name,v.maximum):'—',v.pass?'PASS':'MISS']),eligible=last.checkpoint_eligible===true,promoted=last.checkpoint_promoted===true,level=acceptance.pass?'ok':'warn',headline=acceptance.pass?'All acceptance gates pass':passed+' / '+entries.length+' gates pass',decision=eligible?(promoted?'Eligible and promoted':'Eligible, not promoted'):'Not checkpoint-eligible';if(curr.dynamic){let transition=last.curriculum_transition||{},streak=Number(transition.consecutive_passes??get(last,'competency_curriculum_state.consecutive_passes',0)),required=Number(transition.required_consecutive_passes??curr.active.advance_after_consecutive_passes??1),phaseEpoch=Number(transition.phase_epoch??acceptance.phase_epoch??get(last,'competency_curriculum_state.phase_epoch',0)),maximum=Number(transition.maximum_epochs??curr.active.maximum_epochs??0),state=transition.failed?'Phase refused':transition.complete?'Curriculum complete':transition.advance?'Phase accepted and advancing':acceptance.pass?'Complete pass recorded':'Phase remains active',explanation=transition.failed?'Maximum phase budget reached without satisfying every gate; training stops fail-closed.':transition.advance?'The required consecutive passes were reached, so the next phase may begin.':`A transition needs ${required} consecutive complete passes; current streak ${streak}. This is phase epoch ${phaseEpoch}${maximum?' of at most '+maximum:''}.`;return `<div class="callout ${acceptance.pass?'okay':''}"><span class="pill ${level}">${esc(headline)}</span><p><b>${esc(state)}</b> · ${esc(decision)}.</p><p>${esc(explanation)} Selection ${n(last.selection_metric)}; best ${n(last.best_metric)}.</p></div>`+tbl(rows,['Gate','Observed','Requirement','Result'])}let terminal=acceptance.terminal_epoch===true;return `<div class="callout ${acceptance.pass?'okay':''}"><span class="pill ${level}">${esc(headline)}</span><p><b>Epoch ${n(last.epoch)}</b> · ${terminal?'terminal gate':'interim measurement'} · <b>${esc(decision)}</b>.</p><p>${terminal?'This epoch is an authoritative phase decision.':'The authoritative '+esc(String(curr.active.name||'phase').replaceAll('_',' '))+' decision is at epoch '+n(curr.active.through_epoch)+'.'} Selection ${n(last.selection_metric)}; best ${n(last.best_metric)}.</p></div>`+tbl(rows,['Gate','Observed','Requirement','Result'])}
function requiredPhasePanels(curr){let active=curr.active||{},names=new Set([active.primary_generation_panel||'broad',...Object.keys(active.minimum_generation_accuracy_by_panel||{}),...Object.keys(active.minimum_valid_rate_by_panel||{})]);return [...names].filter(Boolean)}
function validationScopePanel(curr,validationN,contract){let phase=String(curr.active.name||'current phase').replaceAll('_',' '),scratch=contract?.initialization?.mode==='random_scratch_v1',retention=scratch?'Cumulative replay':'Retention versus source baseline',retentionText=scratch?'Each later phase explicitly replays every earlier verified skill bucket; direct panel gates block forgetting.':'The same broad generation panel is compared with the checkpoint before this curriculum. A drop below its protected floor blocks advancement.';return `<div class=scope-list><div class=scope-row><b>1 · Current-phase learning</b>Generated-answer panels required by <b>${esc(phase)}</b>. These determine whether the curriculum may advance.</div><div class=scope-row><b>2 · ${esc(retention)}</b>${esc(retentionText)}</div><div class=scope-row><b>3 · Future curriculum probes</b>Panels for skills assigned to later phases. Low or zero scores are informational now, not failures of the active phase.</div><div class=scope-row><b>4 · Global teacher-forced diagnostics</b>Loss, token accuracy and exact full-trace accuracy across all ${n(validationN)} fixed holdout rows. Useful for diagnosis, but not a claim that every future skill has already been taught.</div></div>`}
function phaseValidationPanel(validation,curr){let active=curr.active||{},panels=validation?.generation_panels||{},primary=active.primary_generation_panel||'broad',names=requiredPhasePanels(curr),rows=names.map(name=>{let panel=panels[name]||{},minAcc=name===primary?active.minimum_generation_accuracy:get(active,'minimum_generation_accuracy_by_panel.'+name,null),minValid=name===primary?active.minimum_valid_rate:get(active,'minimum_valid_rate_by_panel.'+name,null),pass=numeric(panel.accuracy)&&numeric(panel.valid_rate)&&(!numeric(minAcc)||Number(panel.accuracy)>=Number(minAcc))&&(!numeric(minValid)||Number(panel.valid_rate)>=Number(minValid));return [name.replaceAll('_',' '),name===primary?'Primary phase panel':'Required support panel',n(panel.examples),pct(panel.accuracy),numeric(minAcc)?'≥ '+pct(minAcc):'—',pct(panel.valid_rate),numeric(minValid)?'≥ '+pct(minValid):'—',pass?'PASS':'MISS']});return `<p>These generated-answer panels match the active curriculum. Family-level answer and trace gates remain visible in <b>Latest acceptance gate</b>.</p>`+tbl(rows,['Panel','Role','N','Correct answers','Answer gate','Valid format','Format gate','Result'])}
function compareRows(current,baseline){let names=[...new Set([...Object.keys(baseline||{}),...Object.keys(current||{})])];return names.map(name=>{let before=baseline?.[name]||{},now=current?.[name]||{},delta=100*(Number(now.accuracy)-Number(before.accuracy));return [name,n(now.examples??before.examples),pct(before.accuracy),pct(now.accuracy),Number.isFinite(delta)?signed(delta,2,' pp'):'—']})}
function retentionValidationPanel(last,artifact,contract){let baseline=artifact.retention_baseline||{},broad=get(last,'validation.generation_panels.broad',{}),check=get(last,'curriculum_acceptance.checks.broad_retention',{});if(!Object.keys(baseline).length){if(contract?.initialization?.mode==='random_scratch_v1')return '<div class="callout okay"><span class="pill ok">Cumulative replay contract</span><p>There is no inherited checkpoint baseline. Earlier learned panels remain mandatory in later phases, so forgetting blocks phase advancement directly.</p></div>';return '<span class=muted>Waiting for the source-checkpoint retention baseline to load.</span>'}let delta=100*(Number(broad.accuracy)-Number(baseline.accuracy)),pass=check.pass===true,sourceRows=compareRows(broad.by_source,baseline.by_source),difficultyRows=compareRows(broad.by_difficulty,baseline.by_difficulty),familyRows=compareRows(broad.by_family,baseline.by_family).filter(r=>Number((baseline.by_family||{})[r[0]]?.accuracy)>0).sort((x,y)=>parseFloat(x[4])-parseFloat(y[4]));return `<div class="callout ${pass?'okay':''}"><span class="pill ${pass?'ok':'warn'}">${pass?'Retention floor passed':'Retention below floor'}</span><p>Broad generated answers: <b>${pct(broad.accuracy)}</b> versus source baseline <b>${pct(baseline.accuracy)}</b> (${signed(delta,2,' pp')}). Required floor: <b>${numeric(check.minimum)?pct(check.minimum):'—'}</b>.</p><p>This is the forgetting/replay signal. It is separate from whether the current phase itself was learned.</p></div><h3>By source</h3>${tbl(sourceRows,['Cohort','N','Source baseline','Current','Change'])}<h3>By difficulty</h3>${tbl(difficultyRows,['Difficulty','N','Source baseline','Current','Change'])}<details><summary>Previously learned families compared with source baseline</summary>${tbl(familyRows,['Family','N','Source baseline','Current','Change'])}</details>`}
function futureValidationPanel(validation,artifact,curr){let panels=validation?.generation_panels||{},required=new Set([...requiredPhasePanels(curr),'broad']),panelRows=Object.entries(panels).filter(([name])=>!required.has(name)).map(([name,p])=>[name.replaceAll('_',' '),n(p.examples),pct(p.accuracy),pct(p.valid_rate),'Diagnostic only — assigned to a later phase']),baseline=artifact.retention_baseline||{},broad=panels.broad||{},zeroRows=Object.entries(baseline.by_family||{}).filter(([,v])=>Number(v.accuracy)===0).map(([name,v])=>[name,n((broad.by_family||{})[name]?.examples??v.examples),pct((broad.by_family||{})[name]?.accuracy),'Zero at source checkpoint']).sort((x,y)=>parseFloat(y[2])-parseFloat(x[2]));return `<div class=callout><span class="pill warn">Do not read these as current-phase failures</span><p>These panels and families were not required for the active phase. They show transfer and provide an early warning for later curriculum design.</p></div>${tbl(panelRows,['Future panel','N','Correct answers','Valid format','Interpretation'])}${zeroRows.length?'<details><summary>Families that were 0% at the source checkpoint</summary>'+tbl(zeroRows,['Family','N','Current broad probe','Source status'])+'</details>':''}`}
function breakdownTables(validation){let b=validation?.breakdowns||{},total=Object.values(b.by_source||{}).reduce((sum,v)=>sum+Number(v.examples||0),0),make=key=>Object.entries(b[key]||{}).map(([name,v])=>[name,n(v.examples),n(v.language_loss),pct(v.teacher_forced_token_accuracy),pct(v.teacher_forced_sequence_accuracy)]),source=make('by_source'),difficulty=make('by_difficulty'),family=make('by_family');if(!source.length&&!difficulty.length&&!family.length)return '<span class=muted>Aggregate metrics only for this active run. Cohort breakdowns start with the updated trainer.</span>';return `<div class=callout><p>This table teacher-forces the expected trace across all <b>${n(total)}</b> fixed holdout rows, including skills reserved for future curriculum phases. Exact full trace means every supervised output token matched; it is stricter than answer correctness and is diagnostic here.</p></div><h3>By source</h3>${tbl(source,['Cohort','N','Loss','Token','Exact full trace'])}<h3>By difficulty</h3>${tbl(difficulty,['Difficulty','N','Loss','Token','Exact full trace'])}<details><summary>All families</summary>${tbl(family,['Family','N','Loss','Token','Exact full trace'])}</details>`}
...
$('validationScope').innerHTML='<div class=scope-list><div class=scope-row><b>Current trial band</b>Generated answers and exact work determine whether this bounded school band was learned.</div><div class=scope-row><b>Replay retention</b>Previously learned skills are compared with their epoch-0 baseline.</div><div class=scope-row><b>Next-band probes</b>Larger-number tasks are diagnostic only until that band is trained.</div></div>';
$('phaseValidation').innerHTML=tbl(gateRows,['Family','N','Correct answers','Intact format','Exact work','Caps','Gate']);
$('retentionValidation').innerHTML=tbl(replayRows,['Replay family','Baseline','Latest','Change']);
$('futureValidation').innerHTML=tbl(Object.entries(v).filter(([name])=>name.startsWith('next_band/')).map(([name,p])=>[name.slice(10),n(p.examples),pct(p.accuracy),pct(p.valid_rate),'Diagnostic only']),['Future band','N','Correct answers','Valid format','Interpretation']);
...
if(liveGeneration.active){let completed=Number(liveGeneration.completed_panels||0),total=Number(liveGeneration.total_panels||0),examplesDone=Number(liveGeneration.completed_examples||0),examplesTotal=Number(liveGeneration.total_examples||0),ratio=examplesTotal?Math.max(0,Math.min(1,examplesDone/examplesTotal)):0,names=(liveGeneration.panels||[]).filter(x=>x.complete).map(x=>String(x.name||'').replaceAll('_',' ')),scope=liveGeneration.scope==='phase_required_v1'?'active-phase acceptance panels only':'all configured panels',phase=liveGeneration.active_phase?String(liveGeneration.active_phase).replaceAll('_',' '):'';$('summary').innerHTML+=`<div class=card><b>Validation in progress</b><div class="metric ok">${n(completed)} / ${n(total)} panels</div><div class=submetric>${n(examplesDone)} / ${n(examplesTotal)} generated answers · epoch ${n(liveGeneration.epoch)}<br>${esc(scope+(phase?' · '+phase:''))}<br>${esc(names.join(', ')||'Teacher-forced validation running')}</div><div class=bar><i style="width:${100*ratio}%"></i></div></div>`}
...
let accSeries=[{label:'Token accuracy',color:'#60a5fa',values:rows.map(r=>({x:r.epoch,y:Number(get(r,'validation.teacher_forced_token_accuracy',NaN))}))},{label:'Sequence accuracy',color:'#5bd190',values:rows.map(r=>({x:r.epoch,y:Number(get(r,'validation.teacher_forced_sequence_accuracy',NaN))}))},{label:'Generation accuracy',color:'#c084fc',values:rows.map(r=>({x:r.epoch,y:Number(get(r,'validation.generation.accuracy',NaN))}))},{label:'Valid answer rate',color:'#ffc857',values:rows.map(r=>({x:r.epoch,y:Number(get(r,'validation.generation.valid_rate',NaN))}))}],lossSeries=[{label:'Total train loss',color:'#60a5fa',values:rows.map(r=>({x:r.epoch,y:Number(r.train_loss)}))},{label:'Task loss',color:'#22d3ee',values:rows.map(r=>({x:r.epoch,y:Number(r.train_task_loss)}))},{label:'Preservation KL',color:'#c084fc',values:rows.map(r=>({x:r.epoch,y:Number(r.train_preservation_loss)}))},{label:'Validation loss',color:'#ff7d74',values:rows.map(r=>({x:r.epoch,y:Number(get(r,'validation.loss',NaN))}))}];
let trendRows=rows.map(r=>{let panel=get(r,'curriculum_acceptance.primary_panel',primaryPanel);return [r.epoch,n(r.global_step),n(r.train_loss),n(r.train_task_loss),n(r.train_preservation_loss),n(r.preservation_rows),n(get(r,'validation.loss')),pct(get(r,'validation.teacher_forced_token_accuracy')),pct(get(r,'validation.teacher_forced_sequence_accuracy')),panel.replaceAll('_',' '),pct(get(r,'validation.generation_panels.'+panel+'.accuracy')),pct(get(r,'validation.generation_panels.broad.accuracy')),n(r.selection_metric),n(r.best_metric),r.patience??'',lr(r.learning_rate),get(r,'curriculum.phase',''),dur(get(r,'timing.epoch_seconds'))]});
if(d.training_contract?.full_supervision){let names=d.training_contract?.competency_curriculum?['school','wording','generated_foundations','generated_rational','generated_language','deepmind_numeric','deepmind_symbolic','broad']:['school','wording','broad'],colors=['#60a5fa','#ffc857','#5bd190','#c084fc','#ff7d74','#22d3ee','#f472b6','#a3e635'];accSeries=names.map((panel,i)=>({label:panel.replaceAll('_',' ')+' answers',color:colors[i%colors.length],values:rows.map(r=>({x:r.epoch,y:Number(get(r,'validation.generation_panels.'+panel+'.accuracy',NaN))}))}));$('assessment').innerHTML+='<p>Training loss is role-weighted while validation loss is ordinary teacher-forced cross-entropy. Judge capability using generated answers, executable procedural traces, and the separate skill-cohort panels.</p>'}
$('trend').innerHTML='<h3>Accuracy</h3>'+lineChart(accSeries,{zeroOne:true})+'<h3>Loss</h3>'+lineChart(lossSeries)+tbl(trendRows,['Epoch','Step','Total loss','Task loss','Preservation KL','Preserved rows','Global val loss','Global token','Global exact trace','Phase panel','Phase answers','Broad retention','Selection','Best','Patience','LR','Curriculum','Epoch time']);
if(curr.dynamic){$('validationScope').innerHTML=validationScopePanel(curr,validationN,d.training_contract);$('phaseValidation').innerHTML=phaseValidationPanel(v,curr);$('retentionValidation').innerHTML=retentionValidationPanel(last,a,d.training_contract);$('futureValidation').innerHTML=futureValidationPanel(v,a,curr)}else{$('validationScope').innerHTML='<p>The current run predates competency-scoped dashboard reporting. Acceptance and global diagnostics remain available below.</p>';$('phaseValidation').innerHTML='<span class=muted>Current-phase generation panels are unavailable for this run.</span>';$('retentionValidation').innerHTML='<span class=muted>No source-checkpoint retention baseline is attached to this run.</span>';$('futureValidation').innerHTML='<span class=muted>Future curriculum panels are unavailable for this run.</span>'}
$('breakdowns').innerHTML=breakdownTables(v);
let generationPanels=v.generation_panels||{},failureRows=Object.entries(generationPanels).flatMap(([panel,value])=>(value.failure_examples||[]).map(x=>[panel,x.source||'',x.family||'',x.difficulty??'',x.problem||'',x.expected_answer||'',x.parsed_answer??'no valid answer',x.generation||'']));if(!failureRows.length)failureRows=(g.failure_examples||[]).map(x=>['aggregate',x.source||'',x.family||'',x.difficulty??'',x.problem||'',x.expected_answer||'',x.parsed_answer??'no valid answer',x.generation||'']);
let traceFailureRows=Object.entries(generationPanels).flatMap(([panel,value])=>(value.trace_failure_examples||[]).map(x=>[panel,x.family||'',x.operation||'',x.problem||'',x.expected_answer||'',x.parsed_answer??'no valid answer',x.generation||'']));
let panelRows=Object.entries(generationPanels).map(([name,panel])=>[name,panel.split||'',n(panel.examples),pct(panel.valid_rate),pct(panel.accuracy),dur(panel.elapsed_seconds)]);
$('generation').innerHTML=(failureRows.length?tbl(failureRows,['Panel','Source','Family','Difficulty','Problem','Expected','Parsed','Raw generation']):'<span class=muted>No generated-answer failures were recorded in the latest validation.</span>')+(traceFailureRows.length?'<details><summary>Executable-trace failures</summary>'+tbl(traceFailureRows,['Panel','Family','Operation','Problem','Expected answer','Parsed answer','Generated trace'])+'</details>':'')+(panelRows.length?'<details><summary>Raw overview of every generation panel</summary>'+tbl(panelRows,['Panel','Split','N','Valid format','Accuracy','Time'])+'</details>':'');
