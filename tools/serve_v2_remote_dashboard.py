...
for candidate_root in recovery_roots:
    ...
    if candidate_contract and (candidate_root.name.startswith(("math_full_supervision", "math_competency_curriculum")) or str(candidate_status.get("state") or "starting") not in recovery_terminal_states):
...
if recovery_root is not None:
    ...
    training_contract = redact({
        ...
        "full_supervision": recovery_contract.get("format") in {"cftn_full_math_training_v1", "cftn_full_math_training_v2", "cftn_full_math_training_v3"},
        "competency_curriculum": recovery_contract.get("format") in {"cftn_full_math_training_v2", "cftn_full_math_training_v3"},
...
for trial_root in sorted(artifact_root.glob("math_verified_school_trial*"), key=modified_unix, reverse=True):
    if recovery_root is not None and recovery_root.name.startswith(("math_full_supervision", "math_competency_curriculum")):
        break
...
function entrancePanel(a){let report=a.entrance_evaluations||{},items=report.evaluations||[];if(!items.length)return '<p class=muted>Zero-update entrance checks have not completed yet.</p>';let rows=items.map(x=>[String(x.phase||'').replaceAll('_',' '),x.zero_optimizer_updates?'0':'—',x.acceptance?.pass?'PASS':'MISS',x.skipped?'Skipped as already mastered':'First unmet phase — training starts here']);return '<h3>Zero-update phase entrance</h3>'+tbl(rows,['Phase','Optimizer updates','All unchanged gates','Decision'])+'<p class=muted>Only accepted non-final phases may be skipped. The final phase always requires fresh repeated evidence.</p>'}
function competencyAssessment(d,p,a,issues,curr){let rows=a.metrics||[],last=latest(rows);if(!rows.length)return {level:'warn',label:'Too early to assess',detail:'No completed phase validation is available yet.',html:'<span class=muted>Assessment starts after the first completed curriculum validation.</span>'};let acceptance=last.curriculum_acceptance||{},checks=acceptance.checks||{},phaseChecks=Object.entries(checks).filter(([name])=>name!=='broad_retention'),phasePassed=phaseChecks.length>0&&phaseChecks.every(([,value])=>value.pass===true),retention=checks.broad_retention||{},retentionPassed=retention.pass===true,transition=last.curriculum_transition||{},phaseName=String(transition.phase||acceptance.phase||curr.active.name||'current phase').replaceAll('_',' '),streak=Number(transition.consecutive_passes??get(last,'competency_curriculum_state.consecutive_passes',0)),required=Number(transition.required_consecutive_passes??curr.active.advance_after_consecutive_passes??1),phaseEpoch=Number(transition.phase_epoch??acceptance.phase_epoch??get(last,'competency_curriculum_state.phase_epoch',0)),maximum=Number(transition.maximum_epochs??curr.active.maximum_epochs??0),level=issues.length?'bad':transition.failed?'bad':acceptance.pass?'ok':phasePassed&&!retentionPassed?'warn':'warn',label=issues.length?'Infrastructure attention required':transition.failed?'Phase stopped: acceptance failed':transition.advance?'Phase accepted — advancing':acceptance.pass?'All phase and retention gates pass':phasePassed&&!retentionPassed?'Current phase learned; retention below floor':'Current-phase gates still learning',detail=phasePassed&&!retentionPassed?'The skills taught in this phase pass their generation gates, but the broad replay panel has fallen below the protected source-checkpoint floor.':'The curriculum decision is based on generated answers for the active phase plus protected broad retention, not the global 12,000-example teacher-forced table.',next=transition.failed?'The run is fail-closed. Diagnose retention before continuing.':transition.advance?'The next curriculum phase begins from the accepted checkpoint.':`A phase transition needs ${required} consecutive complete passes. Current streak: ${streak}; phase epoch ${phaseEpoch}${maximum?' of at most '+maximum:''}.`;let rowsOut=[['Active phase',phaseName,'Acceptance scope'],['Current-phase checks',phaseChecks.filter(([,v])=>v.pass).length+' / '+phaseChecks.length,phasePassed?'PASS':'Still learning'],['Broad retention',numeric(retention.observed)?pct(retention.observed):'—',numeric(retention.minimum)?'floor '+pct(retention.minimum):'No recorded floor'],['Consecutive complete passes',streak+' / '+required,transition.advance?'Transition approved':'No transition yet']];return {level,label,detail,html:`<div class="callout ${acceptance.pass?'okay':''}"><span class="pill ${level}">${esc(label)}</span><p>${esc(detail)}</p><p><b>Next decision:</b> ${esc(next)}</p></div>`+tbl(rowsOut,['Signal','Observed','Meaning'])}}
...
function validationScopePanel(curr,validationN){let phase=String(curr.active.name||'current phase').replaceAll('_',' ');return `<div class=scope-list><div class=scope-row><b>1 · Current-phase learning</b>Generated-answer panels required by <b>${esc(phase)}</b>. These determine whether the curriculum may advance.</div><div class=scope-row><b>2 · Retention versus source baseline</b>The same broad generation panel is compared with the checkpoint before this curriculum. A drop below its protected floor blocks advancement.</div><div class=scope-row><b>3 · Future curriculum probes</b>Panels for skills assigned to later phases. Low or zero scores are informational now, not failures of the active phase.</div><div class=scope-row><b>4 · Global teacher-forced diagnostics</b>Loss, token accuracy and exact full-trace accuracy across all ${n(validationN)} fixed holdout rows. Useful for diagnosis, but not a claim that every future skill has already been taught.</div></div>`}
...
if(curr.dynamic){$('validationScope').innerHTML=validationScopePanel(curr,validationN);$('phaseValidation').innerHTML=phaseValidationPanel(v,curr);$('retentionValidation').innerHTML=retentionValidationPanel(last,a);$('futureValidation').innerHTML=futureValidationPanel(v,a,curr)}else{$('validationScope').innerHTML='<p>The current run predates competency-scoped dashboard reporting. Acceptance and global diagnostics remain available below.</p>';$('phaseValidation').innerHTML='<span class=muted>Current-phase generation panels are unavailable for this run.</span>';$('retentionValidation').innerHTML='<span class=muted>No source-checkpoint retention baseline is attached to this run.</span>';$('futureValidation').innerHTML='<span class=muted>Future curriculum panels are unavailable for this run.</span>'}
