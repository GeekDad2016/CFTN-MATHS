...
if recovery_root is not None:
    ...
    training_contract = redact({
        ...
        "full_supervision": recovery_contract.get("format") == "cftn_full_math_training_v1",
...
if(d.training_contract?.full_supervision){const m=d.data_manifest||{},bands=['foundations','two digit','three digit'];let previous=0;const phases=curr.phases.map((p,i)=>{const start=previous+1;previous=p.through_epoch;return [p.name.replaceAll('_',' '),start+'–'+p.through_epoch,bands[p.max_school_difficulty-1],n(p.source_quotas?.verified_school_full),n(examples-Number(p.source_quotas?.verified_school_full||0)),i===curr.activeIndex?'Active':i<curr.activeIndex?'Passed':'Scheduled']});$('curriculum').innerHTML=`<p><b>Full repaired math curriculum:</b> ${n(m.train_records)} stored training rows; ${n(m.school_unique_objects)} new school math objects; ${n(m.quarantine?.count)} MathQA program rows quarantined. Original evaluation files are unchanged.</p>`+tbl(phases,['Phase','Epochs','School range','School samples/epoch','Broad replay samples/epoch','State'])+`<p>${n(examples)} sampled examples per epoch, family-balanced within source; repeats are counted explicitly in epoch metrics. Maximum 100 epochs. Gates at 20/40/60 stop the run if mastery is not reached; no automatic downstream release. Loss weights: 50% results, 25% copies, 25% syntax.</p><p>Generation panels: school, unseen wording, and 512 unchanged broad-validation problems. The 12,000-row teacher-forced validation remains separate. Full production acceptance is still required after training.</p>`;$('summary').innerHTML=$('summary').innerHTML.replace('Latest broad generation','Latest curriculum generation')}
...
if(d.training_contract?.full_supervision){accSeries=['school','wording','broad'].map((panel,i)=>({label:panel+' generated answers',color:['#60a5fa','#ffc857','#5bd190'][i],values:rows.map(r=>({x:r.epoch,y:Number(get(r,'validation.generation_panels.'+panel+'.accuracy',NaN))}))}));$('assessment').innerHTML+='<p>For this repaired run, training loss is role-weighted while validation loss is ordinary teacher-forced cross-entropy on the original target format. These losses are not directly comparable. Original-format sequence accuracy can fall when the learned worked trace changes; use the separate generated-answer and exact-school-trace gates to judge capability.</p>'}
